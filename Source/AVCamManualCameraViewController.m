...
@property (nonatomic, weak) IBOutlet UILabel *lblAngle;
        ...
        [self.motionManager startAccelerometerUpdatesToQueue:queue withHandler:^(CMAccelerometerData *accelerometerData, NSError *error)
         {
             dispatch_async(dispatch_get_main_queue(), ^{
                 ...
                 double y = (double)accelerometerData.acceleration.y;
                 double z = (double)accelerometerData.acceleration.z;
                 // projected on sphere
                 y = acos(y) ;
                 y = (y / 3.14151927 ) * 180;
                 y=y-180;
                 y=y*-1;
                 double yangle;
                 yangle=  (round(y*100.0)/100.0);
                 if (z<0)    y=y*-1;
                 NSString *strAngle=[NSString stringWithFormat:@"%.0f°", yangle];

                // NSLog(@"Angle %@", strAngle);
                 self.lblAngle.text=strAngle;
                 if (!isnan(y)) {yangle=y;
...
- (void)configureManualHUD
{
    ...
    self.lensPositionSlider.value = 1.0; // self.videoDevice.lensPosition;
    self.lensPositionSlider.enabled = true;  //( self.videoDevice && self.videoDevice.focusMode == AVCaptureFocusModeLocked && [self.videoDevice isFocusModeSupported:AVCaptureFocusModeLocked] );
...
-  (IBAction)capturePhoto10:(id)sender
...
-(void) SubmitData {
    ...
    [PFGeoPoint geoPointForCurrentLocationInBackground:^(PFGeoPoint *geoPoint, NSError *error) {
      ...
      // DEBUG
        ...
        [iSPEXMeasurement setObject:[NSNumber numberWithDouble: (round(geoPoint.latitude *100)/100.0)     ]  forKey:@"lat"];
        [iSPEXMeasurement setObject:[NSNumber numberWithDouble:(round(geoPoint.longitude *100)/100.0)   ] forKey:@"lon"];
          [iSPEXMeasurement setObject:strUXcurrent forKey:@"ux_datetime"];
       // [iSPEXMeasurement setObject:[NSNumber numberWithDouble:motionManager.accelerometerData.] forKey:@"deviceangle"];
        [iSPEXMeasurement setObject:platform forKey:@"platform"];
        [iSPEXMeasurement setObject:version forKey:@"swversion"];
        // DEBUG
        
       //  [iSPEXMeasurement setObject:[AVCapturePhotoSettings photoSettings] forKey:@"photoSettings"];

        
        [iSPEXMeasurement setObject:@"TEST" forKey:@"product"];
        [iSPEXMeasurement saveEventually];
